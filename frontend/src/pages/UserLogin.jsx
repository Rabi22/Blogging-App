  ...
  return (
    <div style={s.overlay} onClick={close}>
      <div ref={ref} style={{...s.modal, opacity:0}} onClick={e=>e.stopPropagation()}>
        ...
        <button className="btn btn-primary" onClick={close} id="close-user-error-modal" style={{width:'100%',justifyContent:'center'}}>
          Try Again
        </button>
      </div>
    </div>
...
export default function UserLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    const { ok, data, status } = await userAuthAPI.login(fields.email, fields.password);
    setLoading(false);
    if (ok && data.user) {
      login(data.user);
      navigate('/');
    ...
    <main style={s.page}>
      ...
      <div ref={cardRef} style={{...s.card, opacity:0}}>
        <div style={s.head}>
          ...
          <p style={s.sub}>Sign in to share your stories</p>
        </div>
        <form ref={formRef} onSubmit={handleSubmit} style={s.form}>
          <div className="form-group">
            <label className="form-label" htmlFor="user-login-email">Email</label>
            <input id="user-login-email" type="email" className="form-input" value={fields.email}
              onChange={e=>setFields(p=>({...p,email:e.target.value}))} placeholder="you@example.com" required autoFocus />
          </div>
          <div className="form-group">
            <label className="form-label" htmlFor="user-login-password">Password</label>
            <input id="user-login-password" type="password" className="form-input" value={fields.password}
              onChange={e=>setFields(p=>({...p,password:e.target.value}))} placeholder="••••••••" required />
          </div>
          <button type="submit" className="btn btn-primary" disabled={loading} id="user-login-submit-btn"
            style={{width:'100%',justifyContent:'center', background:'linear-gradient(135deg,#10b981,#059669)'}}>
            {loading ? <><span className="spinner" style={{width:16,height:16}}/> Signing in…</> : 'Sign In'}
          </button>
        </form>
        <p style={s.foot}>Don't have an account? <Link to="/register" style={s.link}>Create one</Link></p>
        <p style={{...s.foot, paddingTop: 0, paddingBottom: '28px'}}>
          <Link to="/admin/login" style={{...s.link, color:'#475569', fontSize:'12px'}}>Admin Login →</Link>
        </p>
      </div>
    </main>
  );
...
const s = {
  ...
  card:    {position:'relative',zIndex:1,width:'100%',maxWidth:'420px',background:'#10101e',border:'1px solid rgba(16,185,129,0.18)',borderRadius:'20px',overflow:'hidden',boxShadow:'0 20px 60px rgba(0,0,0,0.6)'},
  ...
  icon:    {width:'52px',height:'52px',borderRadius:'14px',background:'rgba(16,185,129,0.12)',border:'1px solid rgba(16,185,129,0.25)',display:'flex',alignItems:'center',justifyContent:'center'},
  ...
  foot:    {textAlign:'center',fontSize:'13px',color:'#475569',padding:'0 36px 12px'},
