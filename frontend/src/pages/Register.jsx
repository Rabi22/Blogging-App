...
export default function Register() {
  ...
  const [form,setForm] = useState({ username:'', email:'', password:'', bootstrapToken:'' });
  ...
  const handleSubmit = async (e) => {
    ...
    const { ok, data, status } = await authAPI.register(
      form.username,
      form.email,
      form.password,
      form.bootstrapToken
    ...
    <main style={styles.page}>
      ...
      <div ref={cardRef} style={{ ...styles.card, opacity: 0 }}>
        <div style={styles.cardHeader}>
          <Link to="/admin/login" style={styles.backLink}>← Back to Login</Link>
          ...
          <p style={styles.subtitle}>Register as an admin to manage the blog</p>
        </div>
        ...
        <form ref={formRef} onSubmit={handleSubmit} style={styles.form}>
          ...
          <div className="form-group">
            <label className="form-label" htmlFor="reg-bootstrap">Admin Token <span style={{fontWeight:400,color:'#475569'}}>(optional)</span></label>
            <input id="reg-bootstrap" name="bootstrapToken" type="password" className="form-input"
              value={form.bootstrapToken} onChange={handleChange}
              placeholder="Enter admin bootstrap token…" />
            <span style={styles.hint}>Required to register as admin. Leave empty for regular user.</span>
          </div>

          <button type="submit" className="btn btn-primary" disabled={loading}
            id="register-submit-btn" style={{ width:'100%', justifyContent:'center' }}>
        ...
        <p style={styles.footer}>
          ...
          <Link to="/admin/login" style={styles.footerLink}>Sign In</Link>
        </p>
      </div>
    </main>
  );
...
const styles = {
  ...
  card: { position:'relative', zIndex:1, width:'100%', maxWidth:'440px', background:'#10101e', border:'1px solid rgba(99,102,241,0.18)', borderRadius:'20px', overflow:'hidden', boxShadow:'0 20px 60px rgba(0,0,0,0.6)' },
  ...
  iconWrap: { width:'52px', height:'52px', borderRadius:'14px', background:'rgba(99,102,241,0.12)', border:'1px solid rgba(99,102,241,0.25)', display:'flex', alignItems:'center', justifyContent:'center' },
  ...
  footer: { textAlign:'center', fontSize:'13px', color:'#475569', padding:'0 36px 28px' },
